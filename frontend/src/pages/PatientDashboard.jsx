...
import {
  Calendar, FileText, Pill, BrainCircuit, Activity,
  ChevronRight, Sparkles, Plus, Clock, ShieldCheck
} from 'lucide-react';
  ...
  useEffect(() => {
    const fetchDashboard = async () => {
      ...
      try {
        ...
        setError(err.response?.data?.error || 'Failed to load patient dashboard.');
      } finally {
        setLoading(false);
      }
    };
      ...
      >
        <div>
          <h1 className="text-2xl md:text-3xl font-extrabold font-display text-white tracking-tight">
            Patient Health Intelligence Portal
          </h1>
          <p className="text-xs text-slate-400 mt-1">
            Personalized diagnostic insights, upcoming consults & active medication vault
          </p>
        </div>
        <Link
          to="/appointments"
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
        ...
        >
          <div className="flex items-center justify-between mb-3">
            <span className="text-xs font-bold text-slate-400 uppercase tracking-wider">Upcoming Consults</span>
            <div className="p-2 bg-blue-500/10 border border-blue-500/20 text-blue-400 rounded-xl">
              <Calendar className="h-5 w-5" />
            </div>
          </div>
          <div className="text-3xl font-extrabold font-display text-white">{stats?.upcoming_appointments || 0}</div>
          <p className="text-[10px] text-slate-500 mt-2 font-mono">Confirmed doctor appointments</p>
        </motion.div>
        ...
        >
          ...
          <p className="text-[10px] text-slate-500 mt-2 font-mono">Medication directives on file</p>
        </motion.div>
        ...
        >
          <div className="flex items-center justify-between mb-3">
            <span className="text-xs font-bold text-slate-400 uppercase tracking-wider">Medical Reports</span>
          ...
          <p className="text-[10px] text-slate-500 mt-2 flex items-center gap-1">
            <Sparkles className="h-3 w-3 text-cyan-400" /> AI Insights enabled
          </p>
        </motion.div>
        ...
        >
          ...
          <div className="text-3xl font-extrabold font-display text-white">{stats?.completed_appointments || 0}</div>
          <p className="text-[10px] text-slate-500 mt-2 font-mono">Historic consultation records</p>
        </motion.div>
      </div>

      {/* Appointments List */}
      <div className="bg-slate-900/60 backdrop-blur-xl rounded-3xl border border-slate-800/90 p-6 shadow-xl">
        <div className="flex items-center justify-between mb-6">
          <div>
            <h3 className="text-base font-bold font-display text-white">Your Appointments</h3>
            <p className="text-xs text-slate-400">Track consultation status and synthesize AI recommendations</p>
          </div>
        ...
        {!stats?.recent_appointments || stats.recent_appointments.length === 0 ? (
          <div className="text-center py-10 text-xs text-slate-500 border border-dashed border-slate-800 rounded-2xl">
            No appointments found. Click "Schedule Consult" above to book one.
          </div>
        ) : (
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h4 className="text-xs font-bold text-slate-100 group-hover:text-cyan-400 transition-colors">
                      Dr. {apt.doctor_name || apt.doctor?.name || 'Physician'}
                    </h4>
